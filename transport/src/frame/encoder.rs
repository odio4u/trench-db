...
pub fn encode(frame: &frame::frame::Frame) -> Result<Bytes, crate::errors::TransportError> {
    ...
    if frame.payload.len() != frame.header.payload_length as usize {
        return Err(crate::errors::TransportError::InvalidFrame(format!("Payload length mismatch: header says {}, but actual payload is {}", frame.header.payload_length, frame.payload.len())));
    }
